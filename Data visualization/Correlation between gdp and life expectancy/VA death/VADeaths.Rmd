...
이번 시간에는 gl, ordered, rep등의 함수를 이용해 데이터를 내가 원하는 형태로 가공하는 방법과 tidyverse패키지와 magrittr패키지의 %>%연산자를 이용해 데이터를 다루는 방법에 대해 배웠다. 데이터를 분석하거나 시각화할 때에는 데이터를 적절한 형태로 전처리하는 것이 중요다고 생각하는데 이번 강의시간에 배운 패키지를 사용한다면 많은 도움이 될 것 같다고 생각했다. 그리고 1학기때 배웠던 ggplot패키지를 이용해 데이터를 시각화하는 방법과 git사용법, mutate, strsplit 같은 함수에 대해서도 다시 한 번 복습할 수 있어서 도움이 많이 되었다. 또한 하나의 데이터로도 다양한 형태의 barplot을 그릴수 있다는 것을 알게되었고 그렇기 때문에 barplot을 그린다고 하더라도 어떤 barplot이 가장 적절하게 데이터를 시각화를 할 수 있는지 고민해볼 필요도 있다고 생각했다.